...
if __name__ == "__main__":
    ...
    # Generate tasks
    for og in range(n):
        for jter in range(1, num_of_values):
            k = math.exp(0.47 * jter) - 1.0  # Log scale parameter
            if og == 0:
                arr_x.append(k)
    ...
    param_path = []
    hard_set = []
    # Learning rate deviation analysis
    for og in range(n):
        bank = []
        for each in path[og]:
            bank.append(each["lr"])
        param_path.append(statistics.variance(bank))
        hard_set.append(path[og][-1])
    # Store learning rate deviation
    with open("ParamPath.txt", "w") as fp:
        json.dump(path, fp)
    # Log-Log slope computation
    slopes = []
    for og in range(n):        
        slopes.append(log_slope(arr[og], arr_x))
    with open("Slope.txt", "w") as fp:
        json.dump(slopes, fp)
    postprocessing_into_file(
        arr, arr_x, "Rosenbrock function with hyper-optimization", labels
    )
    del arr_x
    arr_x = []
    # Compute algorithms performance with respect to
    arr_robust = [[] for _ in range(n)]
    for jter in range(1, num_of_values):
        k = math.exp(0.47 * jter) - 1.0
        arr_x.append(k)
        for og in range(n):
            arr_robust[og].append(robust_test(labels[og], hard_set[og], k))
    # Log-Log slope computation for robust test
    slopes_rob = []
    for og in range(n):        
        slopes_rob.append(log_slope(arr_robust[og], arr_x))
    with open("Slope.txt", "w") as fp:
        json.dump(slopes_rob, fp)
    postprocessing_into_file(
        arr_robust, arr_x, "Rosenbrock function without hyper-optimization", labels
    )
